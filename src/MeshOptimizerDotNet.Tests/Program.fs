...
// Run with: LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run --project MeshOptimizerDotNet.Tests
module MeshOptimizerDotNet.Tests.Program
...
open MeshOptimizerDotNet.Tests.ObjLoader
...
let testQuantization () =
    ...
    for v in testValues do
        ...
        let fsH = MeshOptimizerDotNet.Quantization.meshopt_quantizeHalf v
        if cppH <> fsH then
    ...
    for h in [| 0us; 0x3C00us; 0xBC00us; 0x3800us; 0x7BFFus; 0x0400us; 0x7C00us; 0x7E00us |] do
        ...
        let fsF = MeshOptimizerDotNet.Quantization.meshopt_dequantizeHalf h
...
let testVertexRemap (mesh: ObjMesh) (verticesPtr: nativeint) (indicesPtr: nativeint) =
    ...
    let fsUnique =
        pinArray fsRemap (fun remapPtr ->
            MeshOptimizerDotNet.IndexGenerator.meshopt_generateVertexRemap
...
let testOptimizeVertexCache (indices: uint32[]) (vertexCount: int) =
    ...
    pinArray indices (fun srcPtr ->
        ...
        pinArray fsDst (fun fsPtr ->
            MeshOptimizerDotNet.VCacheOptimizer.meshopt_optimizeVertexCache
...
let testOptimizeOverdraw (cacheOptIndices: uint32[]) (mesh: ObjMesh) =
    ...
    pinArray cacheOptIndices (fun srcPtr ->
        pinArray (mesh.Vertices) (fun vPtr ->
            ...
            pinArray fsDst (fun fsPtr ->
                MeshOptimizerDotNet.OverdrawOptimizer.meshopt_optimizeOverdraw
    ...
    let fsUnique =
        pinArray fsIndices (fun idxPtr ->
            pinArray fsVerts (fun dstPtr ->
                pinArray (mesh.Vertices) (fun srcPtr ->
                    MeshOptimizerDotNet.VFetchOptimizer.meshopt_optimizeVertexFetch
...
let testAnalysis (indices: uint32[]) (mesh: ObjMesh) =
    ...
    pinArray indices (fun idxPtr ->
        ...
        let fsStats =
            MeshOptimizerDotNet.IndexAnalyzer.meshopt_analyzeVertexCache
        ...
        let fsFetch =
            MeshOptimizerDotNet.IndexAnalyzer.meshopt_analyzeVertexFetch
        ...
        pinArray (mesh.Vertices) (fun vPtr ->
            ...
            let fsOd =
                MeshOptimizerDotNet.Rasterizer.meshopt_analyzeOverdraw
...
let testIndexCodec (indices: uint32[]) (vertexCount: int) =
    ...
    MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexVersion(1)

    let boundSize = int (Native.meshopt_encodeIndexBufferBound(unativeint ic, unativeint vertexCount))
    let fsBoundSize = MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexBufferBound ic vertexCount
    ...
    let fsEncSize =
        pinArray indices (fun idxPtr ->
            pinArray fsBuf (fun bufPtr ->
                MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexBuffer
    ...
    pinArray cppBuf (fun bufPtr ->
        pinArray fsDecoded (fun dstPtr ->
            let rc = MeshOptimizerDotNet.IndexCodec.meshopt_decodeIndexBuffer
...
let testVertexCodec (mesh: ObjMesh) =
    ...
    MeshOptimizerDotNet.VertexCodec.meshopt_encodeVertexVersion(0)

    let boundSize = int (Native.meshopt_encodeVertexBufferBound(unativeint vc, unativeint vs))
    let fsBoundSize = MeshOptimizerDotNet.VertexCodec.meshopt_encodeVertexBufferBound vc vs
    ...
    let fsEncSize =
        pinArray (mesh.Vertices) (fun vPtr ->
            pinArray fsBuf (fun bufPtr ->
                MeshOptimizerDotNet.VertexCodec.meshopt_encodeVertexBuffer
    ...
    pinArray cppBuf (fun bufPtr ->
        pinArray fsDecoded (fun dstPtr ->
            let rc = MeshOptimizerDotNet.VertexCodec.meshopt_decodeVertexBuffer
...
let testSimplify (indices: uint32[]) (mesh: ObjMesh) =
            ...
            pinArray (mesh.Vertices) (fun vPtr ->
                pinArray fsDst (fun dstPtr ->
                    pinArray fsErr (fun errPtr ->
                        MeshOptimizerDotNet.Simplifier.meshopt_simplify
    ...
    pinArray (mesh.Vertices) (fun vPtr ->
        ...
        let fsScale = MeshOptimizerDotNet.Simplifier.meshopt_simplifyScale (NPtr.ofNI<float32> vPtr) vc stride
            ...
            pinArray (mesh.Vertices) (fun vPtr ->
                pinArray fsDst2 (fun dstPtr ->
                    pinArray fsErr2 (fun errPtr ->
                        MeshOptimizerDotNet.Simplifier.meshopt_simplifySloppy
...
let testStripify (indices: uint32[]) (vertexCount: int) =
    ...
    let fsBoundSize = MeshOptimizerDotNet.Stripifier.meshopt_stripifyBound ic
    ...
    let fsStripCount =
        pinArray indices (fun idxPtr ->
            pinArray fsStrip (fun dstPtr ->
                MeshOptimizerDotNet.Stripifier.meshopt_stripify
    ...
    if cppStripCount > 0 then
        ...
        let fsUnstripBound = MeshOptimizerDotNet.Stripifier.meshopt_unstripifyBound cppStripCount
        ...
        let fsUnstripCount =
            pinArray cppSlice (fun srcPtr ->
                pinArray fsUnstrip (fun dstPtr ->
                    MeshOptimizerDotNet.Stripifier.meshopt_unstripify
...
let testSpatialSort (mesh: ObjMesh) =
    ...
    pinArray (mesh.Vertices) (fun vPtr ->
        ...
        pinArray fsRemap (fun fsPtr ->
            MeshOptimizerDotNet.SpatialOrder.meshopt_spatialSortRemap
...
let testMeshlets (indices: uint32[]) (mesh: ObjMesh) =
    ...
    let fsBoundMeshlets = MeshOptimizerDotNet.Clusterizer.meshopt_buildMeshletsBound ic maxVerts maxTris
                ...
                pinArray fsMeshlets (fun mPtr ->
                    pinArray fsMV (fun mvPtr ->
                        pinArray fsMT (fun mtPtr ->
                            MeshOptimizerDotNet.Clusterizer.meshopt_buildMeshlets
        ...
        pinArray mvSlice (fun mvPtr ->
            pinArray mtSlice (fun mtPtr ->
                pinArray (mesh.Vertices) (fun vPtr ->
                    ...
                    let fsBounds = MeshOptimizerDotNet.Clusterizer.meshopt_computeMeshletBounds (NPtr.ofNI mvPtr) (NPtr.ofNI mtPtr) (int m.triangle_count) (NPtr.ofNI<float32> vPtr) vc stride
...
let testIndexSequenceCodec (indices: uint32[]) (vertexCount: int) =
    ...
    let fsBoundSize = MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexSequenceBound ic vertexCount
    ...
    let fsEncSize =
        pinArray indices (fun idxPtr ->
            pinArray fsBuf (fun bufPtr ->
                MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexSequence
                    (NPtr.ofNI bufPtr) boundSize (NPtr.ofNI idxPtr) ic))
    ...
    pinArray cppBuf (fun bufPtr ->
        pinArray fsDecoded (fun dstPtr ->
            let rc = MeshOptimizerDotNet.IndexCodec.meshopt_decodeIndexSequence dstPtr ic 4 (NPtr.ofNI bufPtr) cppEncSize
