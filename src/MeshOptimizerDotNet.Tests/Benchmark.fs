...
// Run with: LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run -c Release --project MeshOptimizerDotNet.Tests -- --bench
module MeshOptimizerDotNet.Tests.Benchmark
...
let createBenchState (gridSize: int) =
    ...
    let idxBound = MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexBufferBound ic vc
    ...
    let vtxBound = MeshOptimizerDotNet.VertexCodec.meshopt_encodeVertexBufferBound vc vs
...
type EncodeIndexBenchmark() =
    ...
    [<Benchmark(Baseline = true)>]
    member this.Cpp() =
        let bound = MeshOptimizerDotNet.IndexCodec.meshopt_encodeIndexBufferBound this.S.IC this.S.VC
...
type DecodeIndexBenchmark() =
    ...
    member this.FSharp() =
        ...
        pinArray this.S.IdxEncBuf (fun bp ->
            pinArray dst (fun dp ->
                MeshOptimizerDotNet.IndexCodec.meshopt_decodeIndexBuffer
...
type EncodeVertexBenchmark() =
    ...
    [<Benchmark(Baseline = true)>]
    member this.Cpp() =
        let bound = MeshOptimizerDotNet.VertexCodec.meshopt_encodeVertexBufferBound this.S.VC this.S.VS
        let buf = Array.zeroCreate<byte> bound
        pinArray this.S.Mesh.Vertices (fun vp ->
            pinArray buf (fun bp ->
...
type DecodeVertexBenchmark() =
    ...
    member this.FSharp() =
        ...
        pinArray this.S.VtxEncBuf (fun bp ->
            pinArray dst (fun dp ->
                MeshOptimizerDotNet.VertexCodec.meshopt_decodeVertexBuffer
...
type BuildMeshletsBenchmark() =
    ...
    member this.Setup() =
        ...
        this.MaxMeshlets <- MeshOptimizerDotNet.Clusterizer.meshopt_buildMeshletsBound this.S.IC maxVerts maxTris
...
type StripifyBenchmark() =
    ...
    member this.Setup() =
        this.S <- createBenchState this.GridSize
        this.StripBound <- MeshOptimizerDotNet.Stripifier.meshopt_stripifyBound this.S.IC
