...
```bash
# Run all benchmarks
LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run -c Release --project src/MeshOptimizerDotNet.Tests -- --bench --filter "*.Benchmark*"

# Run a single function
LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run -c Release --project src/MeshOptimizerDotNet.Tests -- --bench --filter *VertexCache*

# With hardware performance counters (needs perf + paranoid ≤ 1)
LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run -c Release --project src/MeshOptimizerDotNet.Tests -- --bench --counters

# Short run (fewer iterations, faster)
LD_LIBRARY_PATH=/tmp/meshoptimizer/build dotnet run -c Release --project src/MeshOptimizerDotNet.Tests -- --bench --job short
```
